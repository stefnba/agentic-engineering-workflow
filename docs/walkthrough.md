...
- **A sibling ticket landed first and your branch now conflicts with it** — caught either by the
  ...
  spending a round on a diff that can't land as reviewed. Either way it surfaces in the same tab
  you're already in, and you resolve it there — it isn't a separate escalation.
  [Git mechanics](../workflow/git-mechanics.md) (Ticket-branch currency) owns the rule.
